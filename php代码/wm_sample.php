...
where studentsInfo.学号 = studentsBalance.学号 and studentsInfo.住址 = dormsInfo.住址 and studentsInfo.学号 = '201655110215' and 姓名 = '刘腾';";                // 数据库操作sql语句
                            $query = mysql_query($sql);                         // 在数据库中执行sql语句
                            $contentStr = '';
                            $rs = mysql_fetch_array($query);             // 获取结果，并把多个结果一个一个取出，然后加入字符串中
                            $contentStr = $rs[0];
